...
MuseScore {
  ...
  property variant englishWordsStartingWithApostrophe : ["'bout","'cause","'cept","'em","'fro","'n","'neath","'nother","'pon","'re","'round","'s","'sall","'scuse","'sfar","'sup","'thout","'til","'tis","'twas","'tween","'twere","'ve"]
  property variant germanWordsStartingWithApostrophe : []

  function changeQuotesEnglish(text, type) {
    var t = text;
    // Left single quote (exclude words that start with apostrophe)
    if (type <= 1 // lyrics SINGLE or BEGIN syllable, not END or MIDDLE
        && englishWordsStartingWithApostrophe.indexOf(t.replace("&quot;", '"').replace(/[,.?¿!¡;:"‘’“”‚„‹›«»–— *†‡()\[\]{}\/`]/g, "").replace(/'$/, "")) == -1) { // exclude common contractions that starts with an apostrophe (omit punctuation when checking against word list)
      t = t.replace(/^'/g,"‘");
      t = t.replace(/(\s|\s&quot;)'/g, "$1‘");
      t = t.replace(/^&quot;'|^“'/g, "“‘");
      }
  ...
  function changeQuotesGerman(text, type) {
    var t = text;
    // Left single quote (exclude words that start with apostrophe)
    if (type <= 1 // lyrics SINGLE or BEGIN syllable, not END or MIDDLE
        && germanWordsStartingWithApostrophe.indexOf(t.replace("&quot;", '"').replace(/[,.?¿!¡;:"‘’“”‚„‹›«»–— *†‡()\[\]{}\/`]/g, "").replace(/'$/, "")) == -1) { // exclude common contractions that starts with an apostrophe (omit punctuation when checking against word list)
        t = t.replace(/^'/g, "‚");
        t = t.replace(/(\s|\s&quot;)'/g, "$1‚");
        t = t.replace(/^&quot;'|^„'/g, "„‚");
      }
  ...
  function changeQuotes(text, type) {
    if (language.endsWith("German"))
      return changeQuotesGerman(text, type);
    else
      return changeQuotesEnglish(text, type);
  }
